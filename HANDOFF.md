...
Started 2026-08-19, current as of 2026-09-06. **Everything here is measured or
...
| Did my change help? | no numbers exist | 157 labelled cases across three corpora (84 on the ML papers), 4 configurations |
...
- Routes: checked against the dispatch in `serve.py` by `src/check_docs.py`,
  ...
  **Assets:** `/pipeline-map.js` the diagram · `/answer-mark.js` the logic
  that decides which words of a passage are set bold · `/passages.js` which
  passages appear beneath the answer · `/common.js`, `/tokens.css` and
  `/base.css`, the header, colour tokens and base rules the four pages
  share · `/fonts/*`. The four shared files were split out of the pages on
  2026-09-06 and the dispatch did not serve them until 2026-09-07, so for a
  day the live pages loaded unstyled while the recorded build, which serves
  a directory, did not.
  ...
  **Measurements:** `/api/eval`, what `src/evaluate.py` wrote for the active
  corpus · `/api/per-case` and `/api/hard-cases`, the per-question rows and
  the structural fixture for the active corpus · `/api/analytics`, everything
  the analytics page plots, generated by `src/build_analytics.py`. Until
  2026-09-07 the per-case and hard-cases routes served the ML papers' files
  under every corpus, and a third route, /api/threshold, served a
  calibration of the ML gate on a golden set that ended on 2026-08-25, which
  nothing fetched. That route is gone.

  **`/health`** carries the shipped defaults, `k` and `expansion`, and the
  three operator flags `public`, `generate_allowed` and `reindex_allowed`.

  **With `RAG_PUBLIC=1`:** `/api/index/start` and `/api/index/inspect` answer
  403 unless `RAG_ALLOW_REINDEX=1`; `POST /ask` with `generate: true` returns
  the passages and a note unless `RAG_ALLOW_GENERATE=1`; a failed generation
  or attribution is reported as a call that failed, with the detail in the
  server log rather than in the response; and a 400 or 413 sent before the
  body is read closes the connection. Added on 2026-09-06 and 2026-09-07.
...
1. **Closed 2026-08-21. The spreadsheet assumption was wrong.** The loaders
   ...
   reads it back: 33 checks (15 when written, 3 of which failed with the old
   assumption put back).
   ...
   `.docx`/`.pptx`/`.xlsx`/`.pdf`, from Wikipedia) and `data-quant/` (35 arXiv
   q-fin papers; this line said 18 from 2026-08-22 until 2026-09-06, and it was
   wrong when written, because `eval/golden-quant.json` recorded 35 papers in
   the commit of that same day that added it, and `docs/corpus-manifest.md`
   lists the 35). Both are
   gitignored and rebuildable. `RAG_STORE_DIR` now moves
   ...
   **Closed since, and this paragraph was stale until 2026-08-27.** The OCR path
   ...
   golden sets and are measured: 25 + 7 bird cases and 33 + 8 quant since the
   2026-09-01 reclassification moved one bird question and two quant questions
   the documents do not answer into the adversarial half. Until 2026-09-06
   this line still read 26 + 6 and 35 + 6, the counts before that date.
2. **Two structural failure modes, ~10% and ~6%.** Measured on all three
   ...
   configuration. Re-derived after the 2026-09-01 label corrections and
   reclassification, the fixtures hold 3 of 67 ML cases (4.5%), 2 of 25 bird
   cases and 0 of 33 quant cases, in `eval/hard_cases.json`,
   `eval/hard_cases-birds.json` and `eval/hard_cases-quant.json`, and
   `eval/RESULTS.md` quotes the 3 of 67; the 2026-08-27 figures stay here
   because the paragraphs below were written against them. **That floor is
   relative to the first stage, not absolute.**
...
8. **The settings controls re-run retrieval.** Done 2026-08-21. `/api/trace`
   ...
   describe the 84 cases of the ML-paper set (the project measures 157 across
   the three corpora); the two answer different questions on purpose.
   ...
   - `retrieve()` takes a **different path** when reranking is off. It
     ...
     (81 checks). That test is what the module's docstring had claimed existed
...
9. **Two of the three unbuilt journey pieces are still unbuilt:** the pool
   ...
   (84 rows for the ML papers, with the 32 bird and 41 quant rows in
   `eval/per_case-birds.json` and `eval/per_case-quant.json`: retrieved
   passages, per-passage relevance, confidence, and one of
...
Three of them distinguish a check that did not run from one that passed,
since 2026-09-06. `check_golden.py` exits 2 when a configured corpus has no
index or no golden set, naming the ingest command; `test_ocr.py` exits 2
without RapidOCR and `check_docs.py --tests` then reports the suite as NOT
RUN and exits 2 itself; and `build_results_doc.py --check` also fails on a
prose case count that disagrees with the results file, unless the sentence
carries an absolute date and a past-tense verb, or names the other corpus
...
| `docs/polish-backlog.md` | the 22 findings rated polish on 2026-09-05, re-probed 2026-09-14 |
