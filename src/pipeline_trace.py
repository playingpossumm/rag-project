...
# How much text the window PLACEMENT is scored over, as against how much is
# shown. See the note inside _brief: these were one number and separating
# them is what let the display grow without moving every window to offset 0.
# 420 because that is the value the placement was last tuned at, on
# 2026-09-02; the docstring's 260 describes an older display limit.
_SCORE_SPAN = 420


def _brief(chunk: dict, query: str = "", limit: int = 720) -> str:
    ...
    **The limit was 420 until 2026-09-03, and it was costing answers.** The
    window is placed by where the question's words fall, which is the best
    signal available and is exactly wrong for a definitional question: "what is
    X called" asks for the word it does not contain, so the window centres on
    the question's vocabulary and the answer term sits outside it. Measured
    across all three corpora, 21 questions retrieved the answer and displayed a
    window without it, which to a reader is indistinguishable from not finding
    it. Answer visible in the excerpt, against the whole chunk as the ceiling:
    ...
    720 shows 99% of what the whole chunk offers and averages 591 characters,
    which is the paragraph this excerpt was always meant to be. Measured after
    the two defects below were fixed, not before: the first table produced here
    was generated by the version that still scored placement over `limit`, and
    it understated every row above 420 because the window was drifting to the
    head of the passage as the limit grew.
    """
    text = _strip_apparatus(" ".join(chunk["text"].split()))
    # No early return for a passage that already fits. There used to be one,
    # and raising the limit to 720 turned it into the very defect this function
    # exists to prevent: a chunk of 498 characters opening on a title block and
    # author addresses now fitted inside the limit, so it was handed back whole
    # and the excerpt opened on metadata again. Four checks in test_excerpt.py
    # caught it. The window is chosen the same way whatever the length, and a
    # passage whose first sentence is the best one is returned unchanged
    # anyway, because it wins the same test.
    wanted = _terms(query)
    if not wanted:
    ...
    # Scored over a FIXED span, not over `limit`. The two were the same number
    # until 2026-09-03, and that hid a discrepancy between this function and
    # its own docstring: the docstring says the window opens at the sentence
    # with the most question words IN IT, and the code scored everything the
    # window would show. While the limit was small the two nearly agreed.
    # Raising it to 720 pulled them apart, because a window starting at offset
    # 0 then contains every question term the passage has, so the title block
    # wins on every question and the excerpt opens on metadata again -- the one
    # thing this function exists to prevent. Four checks in test_excerpt.py
    # caught it.
    #
    # So the placement of the window and the size of the window are separate
    # concerns now. 420 is the span the placement was tuned at and it stays
    # there; `limit` only decides how much is shown once the start is chosen.
    best, best_score = 0, -1
    for i, start in enumerate(starts):
        window = text[start:start + _SCORE_SPAN]
