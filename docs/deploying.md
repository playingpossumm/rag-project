...
payload `/api/chat` returns for each — every retrieval stage with its ranking
and scores, the fusion arithmetic, what the reranker moved, what the cap
dropped, the verdict and its threshold, the term attribution. Recording the ten
...
by a frontier model, and a reader is entitled to know which they are reading.
...
then Vercel does not work — for reasons of arithmetic rather than taste. The
